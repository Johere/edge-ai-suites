...
# 看 task 列表（这也是 multilevel 起来后的第一件事该检查的）
curl -sf http://localhost:8192/v1/tasks | jq '.tasks[].name'
# 当前镜像的 builtin 通常只有：summary, summary_zh
# fridge / child_safety / elder_wakeup 等 use case task 需要按 §3 动态注册
```
...
**本 repo 里的 use case task 都需要动态注册**（当前 multilevel 镜像通常只内置 `summary` / `summary_zh`）：
...
如果只跑基线（fridge / child_safety / elder_wakeup），至少要注册前三个 task。否则 MCP task-poller 调 `/v1/summary` 时会遇到 `Unsupported summarization task: <name>`。
...
UC="$UC" TASK="$TASK" PROMPT_MD="$PROMPT_MD" python3 - <<'PY' > /tmp/register-${UC}.json
...
body = {
    ...
    "mode": "full",
    "description": f"Auto-registered from {os.environ['PROMPT_MD']}",
    "content": {"text": text},
}
...
echo "== payload =="
jq '{task_name, mode, description, content_len:(.content.text|length)}' /tmp/register-${UC}.json

HTTP=$(curl -sS -o /tmp/resp-${UC}.json -w "%{http_code}" \
  -X POST ${VLM}/v1/tasks \
  -H 'Content-Type: application/json' \
  --data-binary @/tmp/register-${UC}.json)
...
if [[ "$HTTP" == "409" ]]; then
  ...
  cat /tmp/resp-${UC}.json | jq .
else
  echo "  $TASK FAILED (HTTP $HTTP):"
  cat /tmp/resp-${UC}.json
  exit 1
fi

# 校验注册成功
curl -sS ${VLM}/v1/tasks | jq ".tasks[] | select(.name==\"${TASK}\")"
```

**注意 task_name 命名规则**（服务端 pydantic 校验）：
- 全小写 ascii，2-64 字符，允许下划线
- 正则 `^[a-z][a-z0-9_]{1,63}$`
- **不能与现有 task 重名**。当前 builtin 通常是 `summary` / `summary_zh`；若 task 已存在，POST 返回 409，可用 PATCH 更新。

### 3.2 一键注册全部 use case task（本 repo 常用，脚本可直接跑）

当前 multilevel 镜像通常只内置 `summary` / `summary_zh`，所以 fridge 也要动态注册。下面脚本会批量注册 fridge / child_safety / elder_wakeup / high_altitude_safety / parking_safety；若只跑基线，保留前三个即可。
...
register_one () {
    ...
    local PROMPT_MD=${REPO}/use-cases/${UC}/${PROMPT_FILE}
...
for pair in \
    ...
    "parking_safety:parking_safety_monitor:prompt.md"; do
    IFS=: read -r UC TASK PROMPT_FILE <<< "$pair"
    echo "=== $UC → $TASK ==="
    register_one "$UC" "$TASK" "$PROMPT_FILE"
done

# 校验：应看到 2 个 builtin + 5 个 dynamic（若只跑基线，则至少 2 builtin + 3 dynamic）
...
- **当前 task 列表（运行时 source of truth）**：`GET http://localhost:8192/v1/tasks`。不同 multilevel 镜像版本的 builtin 可能不同，本指南按当前镜像仅内置 `summary` / `summary_zh` 处理。
