...
VLM 由两个 Docker 容器组成，分别在 `smart-community/docker/` 下用两份 compose 文件管理，
`multilevel-video-understanding/compose.yaml` 通过 `depends_on` 编排它一起起：

```
smart-community/docker/
...
| 可选：swap 32 GB | 35B FP8 编译期内存高峰会 OOM，用 `agent-ai.smarthome/scripts/helpers/setup-swap-32g.sh` |
...
> 若之前跑过 `agent-ai.smarthome/start-video-summary-service/end2end` 那个旧
> stack，先停掉它避免端口 (`:41091` / `:8192`) 冲突：
>
> ```bash
...
git -C /home/user/jie/smarthome/smart-community submodule update --init --recursive
...
```bash
cd /home/user/jie/smarthome/smart-community/docker/vllm-serving
...
修改 `smart-community/docker/multilevel-video-understanding/set_env.sh`：
...
REPO=/home/user/jie/smarthome/smart-community
...
grep -A2 "path_remap" /home/user/jie/smarthome/smart-community/config.yaml.example
```
...
- **VLM 服务源码 / compose（当前使用）**：`smart-community/docker/multilevel-video-understanding/` +
  `edge-ai-libraries/microservices/multilevel-video-understanding/`（submodule）
- **旧 stack（已弃用）**：`agent-ai.smarthome/start-video-summary-service/end2end/`——如果之前跑过，
