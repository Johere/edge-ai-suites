...
#### Schema 声明（YAML）—— 归属每个 use case

**没有全局共享 schema。** `video_summary_tasks` 是单张共享表，但"哪些扩展列参与哪个 use case 的 pipeline"由每个 use case 在自己 `use_case_dict.<uc>.schema` 下声明。一个 use case 只声明并解析它自己需要的字段，专属字段（如 `pet_zone`）不会污染其它 use case。
    ...
    schema:
      video_summary_tasks:
        extensions:
          ...
          - { name: "severity", type: "text", required: false }
          - { name: "desc",     type: "text", required: true }
          - { name: "pet_zone", type: "text", required: false }  # 专属字段，仅 pet_safety 解析
      custom_tables: []
  fridge:                                # report-only：无扩展字段
    schema:
      video_summary_tasks: { extensions: [] }
      custom_tables: []
```

`alerts` 表保持 use-case-agnostic（各 use case 默认不声明扩展）。

#### Schema Manager 行为（`schema-manager.ts`）

1. MCP Server 启动时，**遍历每个 use case 的 `schema`**，对比声明与实际 DB 列
2. 新增字段：执行 `ALTER TABLE ADD COLUMN`（幂等；多个 use case 声明同名列如 `event`/`desc` 只加一次）
3. 已移除字段：保留在 DB 中（非破坏性），应用层忽略
4. 类型变更：警告并要求手动迁移

#### Schema ↔ Prompt 校验（按 use case）

某 use case 里 `required: true` 的扩展字段，必须出现在其 VLM task 的 LOCAL_PROMPT 输出格式中。`smartbuilding_use_case_validate` 只用**该 use case 自己的** schema 做校验，互不干扰：

```
child_safety required fields: [event, desc]     (severity optional)
     ↓ validate (case-insensitive search in its LOCAL_PROMPT)
