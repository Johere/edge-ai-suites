...
> 想让 fridge 即使遇到 critical 也恒不告警，给 `use_case_dict.fridge` 补 `rules`（`excludeEvents` 覆盖会出现的 event 名，或把 `severityThreshold` 抬到 `critical` 以上没有的级别行不通——只能靠 `excludeEvents`），或者保持 `prompt.md` 不产出 `SEVERITY` 行。测试完记得 `DELETE FROM video_summary_tasks WHERE id=<TASK_ID>` 清理这条假数据。
...
```bash
export SMARTBUILDING_DATA_DIR="${SMARTBUILDING_DATA_DIR:-$HOME/.mcp-smartbuilding}"
# 取 loop 里最新一条 critical task 当例子
...
mcp_call smartbuilding_rule_eval "$(jq -n --argjson t "$TASK_ID" '{monitor_id:"cam_child",task_id:$t,create_alert:true}')" \
  | jq '.result.content[0].text | fromjson | {alert_created, alert_id, suppressed_by_cooldown}'

# 立即第二次（间隔 < cooldownSeconds）
mcp_call smartbuilding_rule_eval "$(jq -n --argjson t "$TASK_ID" '{monitor_id:"cam_child",task_id:$t,create_alert:true}')" \
  | jq '.result.content[0].text | fromjson | {alert_created, suppressed_by_cooldown}'
```
...
本节以 `pet_safety` 为例走一遍完整链路，验证 Design §5.2 的承诺：**新增一个 use case 不改 TypeScript、不手写 YAML、不手工注册 VLM task、不写 Python override**。简单阈值类 use case 只靠 `rules` 字典和内置 `defaultRuleEvaluator` 完成规则判断。
...
`high_altitude_safety`（高空抛物）和 `parking_safety`（违章停车）与 `pet_safety` 一样，是**仓库已备 `prompt.md`、零 Python override** 的扩展 use case，用来进一步验证 use case adapter 的扩展点：

- `high_altitude_safety` 验证 `rules.requireDirection`（方向约束）
- `parking_safety` 验证 `rules.alertMessageExtraField` + `rules.excludeZones`（zone 拼接进 alert 消息 + zone 白名单短路）
...
#### 9.7.1 high_altitude_safety（验证 `rules.requireDirection`）

**注册（零 Python，靠 `rules.requireDirection`）**：
...
mcp_call smartbuilding_use_case_register "$(jq -n --arg pt "$PROMPT_TEXT" '{
  ...
  rules:{ severityThreshold:"warn", requireDirection:"downward", cooldownSeconds:30 },
...
**反例验证 `requireDirection` 短路**（仓库无 upward 真实视频，手塞一条 upward task 再 dry-run，印证核心代码只从 `payload.rules.requireDirection` 读约束）：
...
mcp_call smartbuilding_rule_eval "$(jq -n --argjson t "$TASK_ID" '{monitor_id:"cam_high_altitude",task_id:$t}')" \
  | jq '.result.content[0].text | fromjson | {rule_result, requireDirection: .payload.rules.requireDirection}'
# 期望 rule_result.shouldAlert=false，requireDirection=downward
# 清理：sqlite3 "$SMARTBUILDING_DATA_DIR/smartbuilding.db" "DELETE FROM video_summary_tasks WHERE id=$TASK_ID;"
```

#### 9.7.2 parking_safety（验证 `rules.excludeZones` + `alertMessageExtraField`）
...
mcp_call smartbuilding_use_case_register "$(jq -n --arg pt "$PROMPT_TEXT" '{
  ...
  rules:{ severityThreshold:"warn", alertMessageExtraField:"parking_zone", cooldownSeconds:600 },
  summarize:{ method:"SIMPLE", processor_kwargs:{ levels:1, level_sizes:[-1], process_fps:2 } },
  prompt_text:$pt,
  schema_extensions:[
...
mcp_call smartbuilding_use_case_register "$(jq -n --arg pt "$PROMPT_TEXT" '{
  ...
  video_summary_task:"parking_safety_monitor",
  description:"Parking violation detection (dynamic)",
  rules:{ severityThreshold:"warn", alertMessageExtraField:"parking_zone",
          excludeZones:["handicapped"], cooldownSeconds:600 },
  summarize:{ method:"SIMPLE", processor_kwargs:{ levels:1, level_sizes:[-1], process_fps:2 } },
  prompt_text:$pt,
  schema_extensions:[
    { name:"parking_zone", type:"text", required:false },
...
mcp_call smartbuilding_rule_eval "$(jq -n --argjson t "$TASK_ID" '{monitor_id:"cam_parking",task_id:$t}')" \
  | jq '.result.content[0].text | fromjson | {rule_result, excludeZones: .payload.rules.excludeZones}'
# 期望 rule_result.shouldAlert=false，excludeZones=["handicapped"]
# 清理：sqlite3 "$SMARTBUILDING_DATA_DIR/smartbuilding.db" "DELETE FROM video_summary_tasks WHERE id=$TASK_ID;"
```
