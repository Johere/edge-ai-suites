...
| 版本 | 日期 | 更新人 | 说明 |
|------|------|--------|------|
| v0.3 | 2026-07-01 | Li, Jie6 | Phase 9：`pipeline.roi` 子块 + `trajectory_region` webhook 字段 + `<clip>_input.mp4` 自动生成 + `auto_split_area` 早切 |
...
- **ROI crop 生成**：`_emit_segment` 里 prefilter PASS + `roi.enabled` + `trajectory_region_xyxy` 非空时，调 [`prepare_roi_segment()`](../../videostream-analytics/stream_monitor/pipeline/roi_processor.py) 写 `<clip>_input.mp4`；支持三种 mode：
...
| `segment` | `max_duration=10.0`, `min_duration=1.0` |
| `prefilter` | `enabled=false`, `model_path=""`, `target_classes=["person"]`, `min_confidence=0.4`, `min_frames_hit=2`, `detect_fps=2.0`, `device="CPU"` |
| `roi` (Phase 9) | `enabled=false`, `mode="crop"`, `expand=0.25`, `auto_split_area=0.0` |
