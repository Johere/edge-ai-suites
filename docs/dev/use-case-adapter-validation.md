...
> **状态**：`high_altitude_safety` / `parking_safety` 已定为**非默认 use case**，
> 其 `use_case_dict` / monitor / stream / schema 字段（`motion_direction` / `parking_zone`）已从
> `config.yaml.example` / `monitors.yaml.example` / `demo-videos/streams.yaml` **移除**，收录到
> [use-cases/README.md](../../use-cases/README.md) 的 "Extended use cases" 一节。本文档保留为**历史验证记录**
> ——证明 adapter 框架能承载"新 schema 字段 + 新 rules 语义 + 新 cooldown"的扩展 case。想跑一个**默认**
> use case 的从零端到端，见 [use-case-register-verification.md §7](./use-case-register-verification.md)（pet_safety.mp4）。
...
| E | use_case_validate 通过 | ✅ | 现状已支持 dynamic task 校验；本轮文档主线未单独补跑该用例，建议后续补回归截图 |
...
3. **补跑 use_case_validate 回归用例** —— 给 §5 里的 check E 补实测证据（当前代码已支持 dynamic task）
...
| 建议 | 状态 | 落地位置 |
|---|---|---|
| #2 `smartbuilding_prompt_lint` tool | ⛔ **已移除（2026-07，迁至 skill）** — lint 曾抽成 `prompt-lint.ts` 并暴露为独立 tool；现已删除，校验规则内联进 `video-summary-prompt-studio` skill 的 invariant（见 [gap-analysis §6.2](./use-case-adapter-gap-analysis.md)） | — |
...
5. ~~**真实 vLLM 对 3 个已实现 UC 做 `generate_prompt` 保真度抽检（dogfood）**~~ —— autogen 已从 MCP 移除（迁至 `video-summary-prompt-studio` skill）；此保真度抽检随之移到 skill 侧
6. ~~**`smartbuilding_prompt_lint` 独立 tool**~~ —— 已移除（2026-07，迁至 skill invariant，见 [gap-analysis §6.2](./use-case-adapter-gap-analysis.md)）
...
## 11. Round 2 Validation — `parking_safety` (违章停车)
...
### 11.3 Adapter Unit 层验证（6/6 pass）
...
| E | use_case_validate 通过 | ✅ 当前代码已支持；本轮未单独补跑 | ✅ 当前代码已支持；本轮未单独补跑 |
